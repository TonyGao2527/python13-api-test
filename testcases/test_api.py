...
from common import contants, logger  # 引入常量
from ddt import ddt, data
from common.mysql import MysqlUtil  # 引入数据库

logger = logger.get_logger(logger_name='api')
...
class APITest(unittest.TestCase):
    ...
    def test_login(self, case):
        ...
        try:
            ...
            logger.infor("第{0}行用例执行结果：PASS".format(case.case_id))
        except AssertionError as e:
            self.do_excel.write_back(case.case_id+1, resp.text, 'FAIL')  # 实例对象已放外面，类变量加self.调用
            logger.info("第{0}行用例执行结果：FAIL".format(case.case_id))
            raise e
    ...
    def test_register(self, case):
        ...
        mysql = MysqlUtil()  # 实例化对象（进行了一次数据库的连接）
        sql = "SELECT MAX(MobilePhone) FROM future.member WHERE MobilePhone LIKE '186%';"  # 传一条sql进来
        max = mysql.fetch_one(sql)[0]  # 找到最大手机号码, 元组里取第1个值
