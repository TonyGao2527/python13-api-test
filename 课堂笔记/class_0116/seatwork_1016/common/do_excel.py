...
class DoExcel:
    ...
    def write_back(self, sheet_name, row, actual, result):
        """写回数据"""
...
# # 类里俩参数，每次只能执行一个表单----华华老师版
# class DoExcel:
#
#     def __init__(self, file_name, sheet_name=None):
#         self.file_name = file_name  # 操作的文件
#         self.sheet_name = sheet_name  # 操作的表单
#         try:
#             self.workbook = load_workbook(self.file_name)  # 实例化一个workbook表单对象
#         except Exception as e:  # 这里 如文件不存在，应加异常处理--可维护、可扩展，如何做？
#             print('错误：', e)
#
#     # 方法一：类与对象 取值
#     def read(self):
#         """读取数据"""
#         sheet = self.workbook[self.sheet_name]  # 获取sheet表单
#         cases = []  # 列表存所有数据
#         for r in range(2, sheet.max_row+1):
#             row_data = Cases()
#             row_data.case_id = sheet.cell(row=r, column=1).value  # 存储case_id
...
#             row_data['method'] = sheet.cell(row=r, column=5).value  # 存储method请求
#             row_data['expected'] = sheet.cell(row=r, column=6).value  # 存储expected期望
#             cases.append(row_data)  # 每一行放入列表中
#         return cases
#     '''
#
#     def write_back(self, row, col, value_n):
#         """写回数据"""
#         sheet = self.workbook[self.sheet_name]
#         sheet.cell(row=row, column=col).value = value_n
#         self.workbook.save(filename=self.file_name)
