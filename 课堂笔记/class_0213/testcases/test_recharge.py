...
class RechargeTest(unittest.TestCase):
    ...
    def test_recharge(self, case):
        ...
        if data_dict['mobilephone'] == '${investor}':
            ...
            print('pre_investor', type(self.pre_investor), self.pre_investor)
            print("data_dict['mobilephone']", type(data_dict['mobilephone']), data_dict['mobilephone'])
        ...
        try:
            ...
            my_logger.error("第{0}用例执行结果：PASS".format(case.case_id))
        except AssertionError as e:
            self.do_excel.write_back(case.case_id + 1, resp.text, 'FAIL')
            my_logger.error("第{0}用例执行结果：FAIL".format(case.case_id))
            raise e
        # my_logger.error('报错了信息：{}'.format(self.e))
        # finally:
